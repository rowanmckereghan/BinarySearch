...
public int binarySearch(int catNumToFind)
{
  int b = 0;
  int c = 0;
  int d = store.length - 1;
  while(b <= d)
  {
    c = (b + d)/2;
    if (store[c].getCatNum() < catNumToFind)
    {
      b = c + 1;
    }
    else if(store[c].getCatNum() > catNumToFind)
    {
      d = c - 1;
    }
    else
    {
      return store[c].getInventory();
    }
  }
  return -1;
}
public int recursiveBinarySearch(int catNumToFind, int nLow, int nHigh)
{
  ...
  if(nLow == nHigh && store[mid].getCatNum() != catNumToFind)
    return -1;
